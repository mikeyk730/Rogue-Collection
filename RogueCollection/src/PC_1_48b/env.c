...
setenv(char *envfile)
{
  ...
  if ((fd = open(envfile, 0))<0) return(ERROR);
  while (FOREVER)
  {
...
putenv(char *label, char *string)
{
  int i;

  for (i = 0; i<MAXEP; i++)
  {
    if (strcmp(label, element[i].e_label)==MATCH) stccpy(element[i].e_string, string, element[i].strlen);
  }
}
