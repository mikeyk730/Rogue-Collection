...
bool SdlKeyboardInput::HasMoreInput()
{
    return true;
}

char SdlKeyboardInput::GetNextChar() {
    return readchar_impl();
}

std::string SdlKeyboardInput::GetNextString(int size) {
    char buf[512];
    getinfo_impl(buf, size - 1);
    return buf;
}

bool SdlKeyboardInput::IsCapsLockOn()
{
    return LOBYTE(GetKeyState(VK_CAPITAL)) != 0;
}

bool SdlKeyboardInput::IsNumLockOn()
{
    return LOBYTE(GetKeyState(VK_NUMLOCK)) != 0;
}

bool SdlKeyboardInput::IsScrollLockOn()
{
    return LOBYTE(GetKeyState(VK_SCROLL)) != 0;
}

void SdlKeyboardInput::Serialize(std::ostream& out)
{ }
